...
#export FILE_PATH="file.json"

FILE_TYPE=text/plain
#export FILE_PATH="file.csv"
export FILE_PATH="${FILE_PATH:-results.csv}"
...
echo "UPLOAD_URL_RESPONSE : $UPLOAD_URL_RESPONSE"
...
echo "UPLOAD_RESPONSE:  uploading file: $UPLOAD_RESPONSE"
...
COMPLETE_RESPONSE=$(curl -X POST https://slack.com/api/files.completeUploadExternal \
...
echo "COMPLETE_RESPONSE: $COMPLETE_RESPOSE"
...
export UTOKEN="${SLACK_USER_TOKEN:-}"
if [[ -z "${UTOKEN}" ]]; then
  echo "Skipping optional force-share step (SLACK_USER_TOKEN not set)"
  return 0
fi
