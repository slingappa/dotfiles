...
clone_if_not_present()
{
  ...
  if [ ! -d $COMP_DIR ]; then
    ...
    if [ -z "$COMP_URI" ]; then
      git clone $RECURSE -b $COMP_BRANCH ${URI}${COMP_DIR}.git | echo true
    else
      git clone $RECURSE -b $COMP_BRANCH ${COMP_URI} | echo true
    fi
...
process_opensbi()
{
  ...
  cd $SBI_DIR
  make CROSS_COMPILE=$CROSS_COMPILE_PREFIX   -j 32 PLATFORM=generic FW_OPTIONS=0x0 FW_PIC=y
